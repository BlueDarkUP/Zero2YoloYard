...
# 为AI功能添加可选导入
try:
    from libs.ai_models import AI_AVAILABLE, ModelManager
    from libs.sam_worker import SamWorker
    from libs.local_tracker_manager import LocalTrackerManager
except ImportError:
    AI_AVAILABLE = False
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        # --- 新增属性 ---
        self.sam_model_manager = None
        self.sam_worker = None
        self.sam_thread = None
        self.aug_worker = None
        self.aug_thread = None
        self.progress_dialog = None
        # --- 新增跟踪器类型和下拉框 ---
        self.tracker_types = ['CSRT', 'KCF', 'MIL', 'Boosting', 'MedianFlow', 'MOSSE', 'TLD']
        self.tracker_combobox = QComboBox()
        self.tracker_combobox.addItems(self.tracker_types)
        self.tracker_combobox.setToolTip("选择一个目标跟踪算法")
        # --- 结束新增属性 ---
        ...
        # 新增信号连接
        self.canvas.pointClickedForSam.connect(self.run_sam_on_points)
        ...
        # --- 新增 Action ---
        augment_dataset = action('&数据集增强...', self.open_augmentation_dialog, 'Ctrl+Shift+B', 'transform',
                                 '对整个数据集进行批量增强')
        sam_mode = action('SAM 辅助标注', self.toggle_sam_mode, 'S', 'magic', '切换到SAM模式 (按Enter确认)',
                          checkable=True)
        sam_mode.setEnabled(AI_AVAILABLE)
        track_next = action('跟踪到下一帧', self.track_to_next_image, 'Ctrl+T', 'track', '自动跟踪标注到下一张图片',
                            enabled=False)

        # --- 为跟踪器下拉框创建一个 QWidgetAction ---
        tracker_action = QWidgetAction(self)
        tracker_action.setDefaultWidget(self.tracker_combobox)

        self.actions = Struct(
            save=save, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all,
            deleteImg=delete_image,
            lineColor=color1, create=create, delete=delete, edit=edit, copy=copy, createMode=create_mode,
            editMode=edit_mode,
            advancedMode=advanced_mode, shapeLineColor=shape_line_color, shapeFillColor=shape_fill_color,
            zoom=zoom, zoomIn=zoom_in, zoomOut=zoom_out, zoomOrg=zoom_org, fitWindow=fit_window, fitWidth=fit_width,
            zoomActions=zoom_actions, lightBrighten=light_brighten, lightDarken=light_darken, lightOrg=light_org,
            lightActions=light_actions,
            fileMenuActions=(open, open_dir, save, save_as, close, reset_all, quit),
            beginner=(), advanced=(), editMenu=(edit, copy, delete, None, color1, self.draw_squares_option),
            beginnerContext=(create, edit, copy, delete),
            advancedContext=(create_mode, edit_mode, edit, copy, delete, shape_line_color, shape_fill_color),
            onLoadActive=(close, create, create_mode, edit_mode, track_next),
            onShapesPresent=(save_as, hide_all, show_all),
            samMode=sam_mode,
            trackNext=track_next,
        ...
        add_actions(self.menus.view, (
            self.auto_saving, self.single_class_mode, self.display_label_option, labels, advanced_mode, None,
            hide_all, show_all, None, zoom_in, zoom_out, zoom_org, None, fit_window, fit_width, None,
            light_brighten, light_darken, light_org))
        add_actions(self.menus.tools, (self.actions.augmentDataset,))  # 新增
        self.menus.file.aboutToShow.connect(self.update_file_menu)

        add_actions(self.canvas.menus[0], self.actions.beginnerContext)
        add_actions(self.canvas.menus[1],
                    (action('&Copy here', self.copy_shape), action('&Move here', self.move_shape)))
        ...
        self.actions.beginner = (
            ...
            sam_mode, track_next, self.actions.trackerAction, None,
        ...
        self.last_open_dir = None
        self.recent_files, self.max_recent = [], 7
        self.line_color, self.fill_color, self.zoom_level, self.fit_window, self.difficult = None, None, 100, False, False

        recent_files_list = settings.get(SETTING_RECENT_FILES)
        if recent_files_list:
            self.recent_files = [ustr(i) for i in recent_files_list] if have_qstring() else recent_files_list
    ...
    def populate_mode_actions(self):
        ...
        # --- 修改下面这行 ---
        add_actions(self.menus.edit, actions + (self.actions.samMode,) + self.actions.editMenu)
    ...
    def open_next_image(self, _value=False, track_mode=False):
        # Proceeding next image without dialog if having any label
        if not track_mode:  # 在跟踪模式下跳过检查
            if self.auto_saving.isChecked():
                if self.default_save_dir is not None:
                    if self.dirty is True:
                        self.save_file()
    ...
    def open_augmentation_dialog(self):
        try:
            from libs.augmentation_dialog import AugmentationDialog
            from libs.augmentation_worker import AugmentationWorker
        except ImportError as e:
            self.error_message("导入错误", f"无法加载增强模块。请确保已安装所有依赖库: {e}")
            return
        ...
        if not config or not config.get('source_dir') or not config.get('output_dir'):
            QMessageBox.warning(self, "输入错误", "源目录和输出目录都必须设置。")
            return
        ...
        self.aug_thread = QThread()
        self.aug_worker = AugmentationWorker(config)
        self.aug_worker.moveToThread(self.aug_thread)

        self.aug_worker.progress.connect(self.progress_dialog.setValue)
        self.aug_worker.log.connect(self.status)
        self.aug_worker.finished.connect(self.on_augmentation_finished)
        self.progress_dialog.canceled.connect(self.aug_worker.stop)
    ...
    def on_augmentation_finished(self, message):
        ...
        if self.aug_thread:
            self.aug_thread.quit()
            self.aug_thread.wait()
        QMessageBox.information(self, "完成", message)

    def toggle_sam_mode(self, checked):
        if not AI_AVAILABLE:
            QMessageBox.warning(self, "依赖缺失", "SAM功能不可用，请安装 PyTorch, ultralytics 和 segment-anything。")
            self.actions.samMode.setChecked(False)
            return

        if checked and self.sam_model_manager is None:
            reply = QMessageBox.question(self, "加载模型",
                                         "首次使用SAM需要下载模型文件（约1GB），这可能需要一些时间。是否继续？",
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if reply == QMessageBox.No:
                self.actions.samMode.setChecked(False)
                return

            self.status("正在加载SAM模型，请稍候...")
            QApplication.setOverrideCursor(Qt.WaitCursor)
            try:
                self.sam_model_manager = ModelManager(model_name='sam_vit_b')
                self.status("SAM模型加载成功。")
            except Exception as e:
                QMessageBox.critical(self, "错误", f"加载SAM模型失败: {e}")
                self.sam_model_manager = None
                self.actions.samMode.setChecked(False)
            finally:
                QApplication.restoreOverrideCursor()

        if self.sam_model_manager:
            self.canvas.set_sam_mode(checked)
            self.actions.createMode.setChecked(False)
            self.actions.editMode.setChecked(not checked)
            if not checked:
                self.set_edit_mode()

    def run_sam_on_points(self, points, labels):
        if not self.image.isNull():
            ...
            img_np = np.frombuffer(buffer, dtype=np.uint8).reshape((qimage.height(), qimage.width(), 4))
            img_np_rgb = cv2.cvtColor(img_np, cv2.COLOR_BGRA2RGB)

            self.sam_thread = QThread()
            self.sam_worker = SamWorker(self.sam_model_manager, img_np_rgb, np.array(points), np.array(labels))
            self.sam_worker.moveToThread(self.sam_thread)

            self.sam_worker.finished.connect(self.on_sam_finished)
            self.sam_worker.error.connect(lambda msg: (self.status(msg), QApplication.restoreOverrideCursor()))
    ...
    def on_sam_finished(self, mask):
        # 1. 从二值掩码中找到边界框
        contours, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            self.status("SAM未能找到有效的对象轮廓。")
            QApplication.restoreOverrideCursor()
            return
        ...
        if text:
            # --- 关键修复：创建一个形状元组并使用 load_labels ---
            points = [(x, y), (x + w, y), (x + w, y + h), (x, y + h)]
            shape_to_load = (text, points, None, None, False)
            self.load_labels([shape_to_load])
            self.set_dirty()
            # --- 修复结束 ---
    ...
    def track_to_next_image(self):
        if not self.file_path or not self.m_img_list or not self.canvas.shapes:
            QMessageBox.warning(self, "无法跟踪", "请先打开一个目录，并在当前图像上创建至少一个标注。")
            return
        ...
        selected_tracker_type = self.tracker_combobox.currentText()
        tracker_manager = None
        successful_inits = 0

        self.status(f"正在使用 {selected_tracker_type} 跟踪器进行初始化...")
        QApplication.setOverrideCursor(Qt.WaitCursor)

        try:
            current_image_bgr = cv2.imread(self.file_path)
            if current_image_bgr is None:
                raise FileNotFoundError(f"无法读取图片: {self.file_path}")

            initial_shapes = [s for s in self.canvas.shapes if s.selected]
            if not initial_shapes:
                initial_shapes = self.canvas.shapes

            try:
                tracker_manager = LocalTrackerManager(tracker_type=selected_tracker_type)
                successful_inits = tracker_manager.init_trackers(current_image_bgr, initial_shapes)
            except Exception as e:
                print(f"创建或初始化 {selected_tracker_type} 跟踪器时失败: {e}")
                QMessageBox.warning(self, "跟踪器创建失败",
                                    f"无法创建 '{selected_tracker_type}' 跟踪器。\n\n"
                                    f"请确保 'opencv-contrib-python' 已正确安装。\n错误: {e}")
                QApplication.restoreOverrideCursor()
                return

            if successful_inits == 0:
                QMessageBox.warning(self, "跟踪初始化失败",
                                    f"无法在当前选定的边界框上初始化 '{selected_tracker_type}' 跟踪器。\n\n"
                                    "建议：请尝试画一个更紧凑、更能凸显物体特征的边界框，或更换跟踪算法。")
                QApplication.restoreOverrideCursor()
                return

            self.status(f"使用 {selected_tracker_type} 跟踪 {successful_inits} 个对象...")
            if self.auto_saving.isChecked(): self.save_file()
            self.open_next_image(_value=False, track_mode=True)
            QApplication.processEvents()

            if self.file_path is None or current_index == self.m_img_list.index(self.file_path):
                QApplication.restoreOverrideCursor()
                return

            next_image_bgr = cv2.imread(self.file_path)
            if next_image_bgr is None:
                raise FileNotFoundError(f"无法读取下一张图片: {self.file_path}")

            predicted_bboxes = tracker_manager.update_trackers(next_image_bgr)
            ...
            for item in predicted_bboxes:
                ...
                points = [(x, y), (x + w, y), (x + w, y + h), (x, y + h)]
                label = item['label']
                shapes_to_load.append((label, points, None, None, False))

            if shapes_to_load:
                self.load_labels(shapes_to_load)
                self.set_dirty()

            self.status(f"跟踪完成，找到 {len(predicted_bboxes)} 个对象。")
        except Exception as e:
            import traceback
            print(traceback.format_exc())
            QMessageBox.critical(self, "跟踪失败", f"发生错误: {str(e)}")
        finally:
            QApplication.restoreOverrideCursor()
