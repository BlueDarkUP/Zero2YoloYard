...
def build_episode_loader(cfg, k_shot, m_query, episode_type='one_class', num_episodes=-1, phase='train', is_distributed=False, **kwargs):
    ...
    if phase == 'train':
        sampling_strategy = cfg.DATASET.SAMPLING_STRATEGY.TRAIN
    else:  # 'val' or 'test'
        sampling_strategy_at_test = cfg.DATASET.SAMPLING_STRATEGY.TEST
        if sampling_strategy_at_test == None:  # if None, use the same as training
            sampling_strategy = cfg.DATASET.SAMPLING_STRATEGY.TRAIN
        else:
            sampling_strategy = sampling_strategy_at_test

    data_manager = DataManager(dataset_meta_info_dict, supercategory_list)
    topk = cfg.DATASET.SAMPLING_STRATEGY.DYNAMIC_IMPORTANCE.TOPK if sampling_strategy == 'dynamic_importance' else None
    episode_list = data_manager.sampling_episodes(k_shot, m_query, episode_type, sampling_strategy, num_episodes, topk=topk)
    ...
    does_shuffle = cfg.DATASET.SAMPLING_STRATEGY.SHUFFLE  # usually True
    drop_last = cfg.DATASET.SAMPLING_STRATEGY.DROP_LAST if (phase == 'train') else True  # usually True
    if is_distributed and (phase == 'train'):
        # Use DistributedSampler for multi-GPU training to ensure each GPU processes different data
        sampler = DistributedSampler(gkd_dataset, shuffle=(phase == 'train') and does_shuffle, drop_last=drop_last)
        gkd_dataloader = DataLoader(
            gkd_dataset, 
            batch_size=num_roll_out, 
            sampler=sampler,
            num_workers=16,  # 4, 8, 16, 32
            pin_memory=True, 
            drop_last=drop_last
        )
    else:
        gkd_dataloader = DataLoader(
            gkd_dataset, 
            batch_size=num_roll_out, 
            shuffle=(phase == 'train') and does_shuffle, 
            num_workers=16,  # 4, 8, 16, 32
            pin_memory=True, 
            drop_last=drop_last
        )
...
def remove_pad_kps(num_kp_original, kps_texts, kps_texts_mask, data_N_first_dim=[], data_N_second_dim=[], data_N_third_dim=[]):
    ...
    for data in data_N_third_dim:
        assert data.shape[2] == N_final, 'The third dim of data should be N_final.'
        _data = data[:, :, :N_real]
