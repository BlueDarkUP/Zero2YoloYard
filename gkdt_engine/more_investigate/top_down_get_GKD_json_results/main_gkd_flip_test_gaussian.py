...
from more_investigate.top_down_get_GKD_json_results.config import update_config, create_loggers
from utils.utils import list2str, load_samples, make_grid_images, image_normalize, AverageMeter, summarize_losses
from more_investigate.top_down_get_GKD_json_results.build_dataset_coco_eval import build_episode_loader, batch_data_to_device, parse_batch_data, mask_for_mix_modal_training, remove_pad_kps
...
#==========
#TODO
from more_investigate.top_down_get_GKD_json_results.write_prediction_to_json import init_coco_prediction_file,add_coco_predictions,save_coco_predictions
import json
#==========
...
def validate(cfg, model, test_loader, **kwargs):
    ...
    #TODO
    # 定义文件路径
    dataset_meta_info = test_loader.dataset.dataset_meta_info  # Only 1 test dataset at a time
    original_gt_file = dataset_meta_info[0]['anno_path']  # e.g., '/project/vonneumann1/cl2025/keypoint_datasets/animal_pose/ap10k/annotations_fskd/ap10k_grounddino_results_v2.json'
    output_pred_file = cfg.OUTPUT_PRED_FILE  # e.g., '/project/vonneumann1/cl2025/GKD/more_investigate/top_down_get_GKD_json_results/result_json/ap10k_grounddino_our_results.json'
    ...
    for batch_i, batch_episodes_data in enumerate(test_loader):
        ...
        if cfg.LOSS.TYPE == 'direct_coord':  # no need to decode
            ...
            #print("maxvals.shape:",maxvals.shape) #maxvals.shape: (12, 17, 1)
            #print("maxvals:",maxvals)
            #print("preds.shape:",preds.shape) #preds.shape: (12, 17, 2)
        ...
        #print("predictions_o.shape:",predictions_o.shape)  #torch.Size([12, 17, 2])
        ...
        add_coco_predictions(
            ...
            maxvals=maxvals.cpu().detach(),
            original_gt_dict=original_gt_dict,
            original_gt_data=original_gt_data
        )
        #==========================================================
    ...
    #==========================================================
    # TODO:
    # 设置 categories 和 images
    pred_data['categories'] = original_gt_data['categories']
    pred_data['images'] = original_gt_data['images']
    # 循环结束后保存最终结果
