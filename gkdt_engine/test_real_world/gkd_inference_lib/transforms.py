...
def resize_rgba_or_rgb_image(in_im, target_shape, resample=PIL.Image.BILINEAR):
    if in_im.mode == 'RGBA':
        R, G, B, saliency_image2 = in_im.split()
        image2 = Image.merge('RGB', (R, G, B))
        
        image2 = image2.resize(target_shape, resample)
        saliency_image2 = saliency_image2.resize(target_shape, resample)
        
        R, G, B = image2.split()
        out_im = Image.merge('RGBA', (R, G, B, saliency_image2))
    else:  # 'RGB' image
        out_im = in_im.resize(target_shape, resample)
    return out_im
...
class RelativeResize(PreprocessAbstract):
    ...
    def __call__(self, image, anns, meta):
        ...
        '''
        # try:
        image = resize_rgba_or_rgb_image(image, target_shape, resample=self.resample)  # modified by Changsheng, 2025.08.19
        # except:
        #     print(f"Target shape: {target_shape}, Original size: {image.size}")
        #     print(anns)
        #     print(meta)
        #     exit(1)
...
class Resize(PreprocessAbstract):
    ...
    def __call__(self, image, anns, meta):
        ...
        # modify annotations
        if anns.get('keypoints') is not None:
            anns['keypoints'][:, 0] *= x_scale  # (anns['keypoints'][:, 0] + 0.5) * scale - 0.5  # N x 3, (x, y, is_visible) at each row
            anns['keypoints'][:, 1] *= y_scale  # (anns['keypoints'][:, 1] + 0.5) * scale - 0.5
        anns['bbox'][0] *= x_scale  # (xmin, ymin, w, h)
        anns['bbox'][1] *= y_scale
    ...
    def __call__(self, image, anns, meta):  # note for this function we didn't record this transform parameters
        ...
        if self.normalize_keypoints == True:
            ...
            if anns.get('keypoints') is not None:
                ...
                anns['keypoints'][:, 1] = (anns['keypoints'][:, 1] / (h) - 0.5) * 2
...
def recover_kps(kps, current_image_length, scale_trans):
    '''
    Recover the transformed keypoints (in output coordinate frame) to the original image coordinate frame
    :param kps: B x M x 2 (range -1~1)
    :param current_image_length: 384
    :param scale_trans: B x 5, (scale, xoffset, yoffset, pad_xoffset, pad_yoffset)
    :return:
    '''
    B = kps.shape[0]
    kps = kps / 2 + 0.5 # 0~1, since our kp's range is -1~1 thus it needs to perform x/2 + 0.5
    # kps *= current_image_length - 1
    kps = kps * current_image_length
...
class RandomCrop_old(PreprocessAbstract):
    def __init__(self, crop_gt_bbox=False):  # random aspect ratio and edge length for crop
        self.crop_gt_bbox = crop_gt_bbox  # if false, randomly crop image; else cropping the bbox directly
    ...
    def __call__(self, image, anns, meta):
        ...
        if self.crop_gt_bbox == False:  # random crop
        ...
        # add codes for bounding and avoiding xmax<=xmin or ymax<=ymin error, 2025.08.28 
        ltrb[0], ltrb[1] = max(ltrb[0], 0), max(ltrb[1], 0)
        ltrb[2], ltrb[3] = min(ltrb[2], w-1), min(ltrb[3], h-1)
        if (ltrb[2] <= ltrb[0]) or (ltrb[3] <= ltrb[1]):
            ltrb = [0, 0, w-1, h-1]  # if error occurs, directly set image's size
...
class RandomCrop(PreprocessAbstract):
    def __init__(self, crop_gt_bbox=False):
        self.crop_gt_bbox = crop_gt_bbox  # True: 直接裁剪bbox区域, False: 基于bbox尺寸从中心扩展裁剪

    def __call__(self, image, anns, meta):
        w, h = image.size  # 原始图像尺寸
        
        # 获取边界框信息并确保在图像范围内
        bbox_x, bbox_y, bbox_w, bbox_h = anns['bbox']
        bbox_xmin = max(int(bbox_x), 0)
        bbox_ymin = max(int(bbox_y), 0)
        bbox_xmax = min(int(bbox_x + bbox_w)+1, w-1)
        bbox_ymax = min(int(bbox_y + bbox_h)+1, h-1)
        
        if self.crop_gt_bbox:
            # 模式1：直接按bbox裁剪
            ltrb = [bbox_xmin, bbox_ymin, bbox_xmax, bbox_ymax]
        else:
            # 模式2：基于bbox尺寸从中心向外扩展裁剪
            # 计算bbox中心点
            bbox_center_x = bbox_xmin + bbox_w / 2
            bbox_center_y = bbox_ymin + bbox_h / 2
            
            # 计算随机裁剪尺寸（1.0-1.5倍bbox尺寸）
            crop_w = min(int(bbox_w * random.uniform(1.0, 1.5)), w)
            crop_h = min(int(bbox_h * random.uniform(1.0, 1.5)), h)
            
            # 从中心点向外扩展，计算裁剪区域
            crop_xmin = int(bbox_center_x - crop_w / 2)
            crop_ymin = int(bbox_center_y - crop_h / 2)
            crop_xmax = crop_xmin + crop_w -1
            crop_ymax = crop_ymin + crop_h -1
            
            # 调整裁剪区域确保不超出图像边界
            crop_xmin = max(crop_xmin, 0)
            crop_ymin = max(crop_ymin, 0)
            crop_xmax = min(crop_xmax, w-1)
            crop_ymax = min(crop_ymax, h-1)
            
            # 最终边界检查
            ltrb = [crop_xmin, crop_ymin, crop_xmax, crop_ymax]
        
        if (ltrb[2] <= ltrb[0]) or (ltrb[3] <= ltrb[1]):
            ltrb = [0, 0, w-1, h-1]  # 无效裁剪时回退到全图

        # 执行裁剪并调整标注
        image = image.crop(ltrb)
        if anns.get('keypoints') is not None:
            anns['keypoints'][:, 0] -= ltrb[0]
            anns['keypoints'][:, 1] -= ltrb[1]
        anns['bbox'][0] -= ltrb[0]
        anns['bbox'][1] -= ltrb[1]
        meta['offset'] += ltrb[:2]
