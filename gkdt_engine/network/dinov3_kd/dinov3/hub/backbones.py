...
def _setup_visual_model_finetuning(model: torch.nn.Module, visual_layer_to_tune: int):
    # print("========== Visual model named parameters ==========")
    # for name, _ in model.named_parameters():
    #     print(name)
    # print("==================================================")

    # visual_layer_to_tune semantics:
    # -1: all tune
    #  0: all freeze
    #  1: final norm tune
    # >=2: final norm + last (n-1) blocks tune
    # ------------------------------------------------
    ...
    # Case 0: all freeze
    # ------------------------------------------------
    if visual_layer_to_tune == 0:
        print("[Config] visual_layer_to_tune = 0 → all frozen")
        return

    # ------------------------------------------------
    # Case -1: all tune
    # ------------------------------------------------
    if visual_layer_to_tune == -1:
        for param in model.parameters():
            param.requires_grad = True
        print("[Config] visual_layer_to_tune = -1 → all parameters are trainable")  
        return

    # ------------------------------------------------
    # Step 2: unfreeze final norm only
    # ------------------------------------------------
    def _unfreeze_module(module, module_name: str):
        for param in module.parameters():
            param.requires_grad = True
        print(f"[Unfreeze] {module_name}")

    # final norm (mandatory for >=1)
    if hasattr(model, "norm") and model.norm is not None:
        _unfreeze_module(model.norm, "model.norm (final norm)")
    else:
        print("[Warning] model.norm not found, skip final norm unfreeze")

    # ------------------------------------------------
    # Case 1: final norm only
    # ------------------------------------------------
    if visual_layer_to_tune == 1:
        print("[Config] visual_layer_to_tune = 1 → final norm only")
        return

    # ------------------------------------------------
    # Step 3: unfreeze last (n-1) transformer blocks
    # ------------------------------------------------
    num_blocks_to_tune = visual_layer_to_tune - 1

    if not hasattr(model, "blocks"):
        print("[Warning] model.blocks not found, skip block unfreeze")
        return

    total_blocks = len(model.blocks)
    if num_blocks_to_tune > total_blocks:
        print(
            f"[Adjust] Requested {num_blocks_to_tune} blocks, "
            f"but model only has {total_blocks}. Clamp to {total_blocks}."
        )
        num_blocks_to_tune = total_blocks

    start_idx = total_blocks - num_blocks_to_tune

    for i in range(start_idx, total_blocks):
        block = model.blocks[i]
        _unfreeze_module(block, f"model.blocks[{i}]")

    print(
        f"[Config] visual_layer_to_tune = {visual_layer_to_tune} → "
        f"final norm + last {num_blocks_to_tune} blocks (out of {total_blocks})"
    )
...
def _make_dinov3_vit(
    *,
    ...
    visual_layer_to_tune: Optional[int] = None,
    **kwargs,
):
    ...
    if visual_layer_to_tune is not None:
        _setup_visual_model_finetuning(model, visual_layer_to_tune)
