...
class TextTower(nn.Module):
    ...
    def forward(self, token_indices: torch.Tensor, cls_token_only=True) -> torch.Tensor:
        text_tokens = self.backbone(token_indices)
        text_tokens = self.head(text_tokens)

        if cls_token_only == True:
            #print("self.tokens_pooler_type",self.tokens_pooler_type) #argmax
            ...
            return features  # num_texts x C
        else:
            return text_tokens  # num_texts x L x C

    def get_cls_tokens(self, text_tokens, token_indices: torch.Tensor):
        if self.tokens_pooler_type == "first":
            features = text_tokens[:, 0]
        elif self.tokens_pooler_type == "last":
            features = text_tokens[:, -1]
